...
public class MakeTags {
    ...
    public static String makeSelectTag(DbConn dbc, String tagName, String sql, String firstOption, String userValue) {
        ...
        public static String makeMetaSelectTag(DbConn dbc, String tagName, String sql, String firstOption, String userValue) {
        ...
        try {
            ...
            ResultSet results = stmt.executeQuery();
             while (results.next()) {
                ...
                sb.append("<option value='");
                String tableName = FormatUtils.formatString(results.getObject(1)); //tableName is unique so serves as unique identifier id
                sb.append(tableName);  //number
                //sb.append(" ");

                
                if(tableName.equals(userValue)){
                    sb.append("' selected =\"selected\">");
                } else {
                    sb.append("'>");
                }
                 System.out.println("tableName: " + tableName + " " + " userVal: " + userValue);
                
                sb.append(FormatUtils.formatString(results.getObject(1)));  //tableName
                sb.append("</option>\n");
                
            }
        } catch (Exception e) {
            // Put error message right in the select tag (easy for JSP programmer to see error).
            sb.append("<option>Exception thrown in makeMetaSelectTag(): " + e.getMessage()+
                    "</option>"); 
        }
