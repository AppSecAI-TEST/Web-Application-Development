...
<%@page language="java" import="dbUtils.*" %> 
        ...
        <%
            ...
            if (msg.length() == 0) {
                ...
                String spaceshipId = request.getParameter("id");
                if(spaceshipId != null && request.getParameter("modelNameInput") == null){
                    retrievedData = SearchSpaceship.findSpaceship(spaceshipId, dbc);
                    System.out.println(retrievedData.toString());
                    
                    inputData.spaceshipId = spaceshipId;
                    inputData.modelName = retrievedData.modelName;
                    inputData.modelYear = retrievedData.modelYear;
                    inputData.modelType = retrievedData.modelType;
                    inputData.maxCapacity = retrievedData.maxCapacity;
                    inputData.spaceshipPrice = retrievedData.spaceshipPrice;
                    inputData.imageUrl = retrievedData.imageUrl;
                    inputData.fuelCapacity = retrievedData.fuelCapacity;
                }
                
                if (request.getParameter("modelNameInput") != null) { // this is postback
                    // package up Customer String data
                    inputData.spaceshipId = request.getParameter("spaceshipIdInput");
                    ...
                    if (errorMsgs.errorMsg.length() == 0) {
                        ...
                        errorMsgs = DbSpMods.update(inputData, dbc); // errorMsgs will hold all validation messags
        ...
        <jsp:include page="headToContent.jsp"/>
            <h2 style="text-align: center;  color: white">Update Spaceship - <span class='headerLink'><a href="other.jsp">List All</a></span></h2>

            <form action ="updateOther.jsp" method="get" class="insertTable">
                <table class="insertTableCenter">
                    <tr>
                        <td>Customer Id</td>
                        <td><input type="hidden"  name="spaceshipIdInput" value="<%=inputData.spaceshipId%>"/><%=inputData.spaceshipId%></td>
