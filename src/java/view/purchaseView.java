...
import model.Purchase.StringData;
import model.Purchase.StringDataList;
...
public class purchaseView {
    ...
    public static String search(String cssTableClass, DbConn dbc, int cusId, int spaceshipId, String startDate, String endDate){
        ...
        try {
            ...
            while (results.next()) {
                sb.append("<tr>");
                sb.append(FormatUtils.formatIntegerTd(results.getObject("buys_id")));  
                String name = FormatUtils.formatString(results.getObject("firstName")) + " " +  FormatUtils.formatString(results.getObject("lastName"));
                sb.append(FormatUtils.formatStringTd(name));
                sb.append(FormatUtils.formatDoubleTd(results.getObject("trans_cost")));
                sb.append(FormatUtils.formatDateTd(results.getObject("trans_date")));
                sb.append(FormatUtils.formatStringTd(results.getObject("trans_descrip")));
                sb.append(FormatUtils.formatIntegerTd(results.getObject("trans_quantity")));
    ...
    private static StringData dbRowToStringData(ResultSet results) {
        ...
        try {  
            sd.purchaseId = FormatUtils.formatInteger(results.getObject("buys_id"));        
            sd.transCost = FormatUtils.formatDouble(results.getObject("trans_cost"));
            sd.transDate = FormatUtils.formatDate(results.getObject("trans_date"));
            sd.transDescrip = FormatUtils.formatString(results.getObject("trans_descrip"));
            sd.transQuantity = FormatUtils.formatInteger(results.getObject("trans_quantity"));
            System.out.println("dbRowToStringData: " + sd.purchaseId + " " + sd.transCost + " " 
                    + sd.transDate + " " +  sd.transDescrip  + " " + sd.transQuantity);
        } catch (Exception e) {
            sd.errorMsg = "Error in PurchaseView.dbRowToStringData: " + e.getMessage();
        }
        return sd;
    }

    public static StringDataList getPurchaseList(DbConn dbc, int cusId, int spaceshipId, String startDate, String endDate) {

        StringDataList purchaseList = new StringDataList(0);
        System.out.println("getPurchaseList() searching for cusId " + cusId 
                + " for spaceshipId " + spaceshipId + " for startDate " 
        ...
        try {
            ...
            StringData purchase = null;
            while (results.next()) {
                purchase = dbRowToStringData(results);
                purchaseList.addPurchase(purchase);
            }
            System.out.println("purchaseList: " + purchaseList.toString());

        } catch (Exception e) {
            purchaseList.dbError = e.getMessage();
        }
        return purchaseList;
    } 
        ...
        try {
            ...
            while (results.next()) {
                ...
                String updateTag = "<td><a href='updateAssoc.jsp?id="+ idVal +"'><img src='pics/update.png'></a></td>";
                sb.append(updateTag);
                String deleteTag = "<td><a href='assoc.jsp?deleteId="+ idVal +"'><img src='pics/delete.png'></a></td>";
                sb.append(deleteTag);
                sb.append(FormatUtils.formatIntegerTd(results.getObject("buys_id")));             
                sb.append(FormatUtils.formatIntegerTd(results.getObject("customer_id")));
                sb.append(FormatUtils.formatIntegerTd(results.getObject("spaceship_id")));
            ...
            return "Exception thrown in purchaseView.listWithDelete(): " + e.getMessage()
                    + "<br/> partial output: <br/>" + sb.toString();
        }
    }
