...
<%@page language="java" import="dbUtils.*" %> 
        ...
        <%
            ...
            if (msg.length() == 0) {
                ...
                if(userId != null && request.getParameter("customerEmailInput") == null){
                    retrievedData = SearchCustomer.findUser(userId, dbc);
                    System.out.println(retrievedData.toString());
                    
                    inputData.customerId = retrievedData.customerId;
                    inputData.customerEmail = retrievedData.customerEmail;
                    inputData.customerPassword = retrievedData.customerPassword;
                    inputData.customerPassword2 = retrievedData.customerPassword;
                    inputData.customerUsername = retrievedData.customerUsername;
                    inputData.firstName = retrievedData.firstName;
                    inputData.lastName = retrievedData.lastName;
                    inputData.customerDOB = retrievedData.customerDOB;
                    inputData.role = retrievedData.role;
                    inputData.roleId = retrievedData.roleId;
                }
                ...
                roleSelectTag = MakeTags.makeSelectTag(dbc, roleSelectTagName, roleSelectSql,
                        roleFirstOption, inputData.roleId);
                ...
                if (request.getParameter("customerEmailInput") != null) { // this is postback
                    ...
                    inputData.customerId = request.getParameter("customerIdInput");
                    ...
                    if (errorMsgs.errorMsg.length() == 0) {
                        ...
                        //TODO: add update
                        errorMsgs = DbCusMods.update(inputData, dbc); // errorMsgs will hold all validation messags
        ...
        <jsp:include page="headToContent.jsp"/>
            <h2 style="text-align: center; color: white">Update Customer - <span class='headerLink'><a href="users.jsp">List All</a></span></h2>

            <form action ="updateUser.jsp" method="get" class="insertTable">
                <table class="insertTableCenter">
                    <tr>
                        <td>Customer Id</td>
                        <td><input type="hidden"  name="customerIdInput" value="<%=inputData.customerId%>"/><%=inputData.customerId%></td>
